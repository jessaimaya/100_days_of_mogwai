            ...
            <section class="wrapper">
                <div class="rmg" patch:children=rx.branch_map(move |Out::CurrentState(st)|
                    Patch::Replace{value: match st {
                        ...
                        In::DisplayRecipe(recipe) => display_recipe(&new_tx, recipe.to_owned()),
...
fn get_ingredient_tpl(
    ingredient: Option<String>,
    measure: Option<String>,
) -> Option<ViewBuilder<HtmlElement>> {
    if let (Some(ing), Some(quantity)) = (ingredient, measure) {
        return Some(builder!(
            <li>
                <span class="title">{format!("{}: ",ing)}</span>
               <span class="measure">{quantity}</span>
            </li>
        ));
    }
    None
